...
BEGIN
  ...
  -- Assign technician role by default
...
-- Also allow profile insert for the trigger (service role)
CREATE POLICY "Service can insert profiles" ON public.profiles
  FOR INSERT
  WITH CHECK (true);

-- RLS on existing tables: scope to authenticated + active users
-- We need a function to check if user is active
